...
namespace Helios.Messages.Incoming
{
    class RequestBuddyMessageEvent : IMessageEvent
    {
        public void Handle(Avatar avatar, Request request)
        {
            ...
            {
                avatar.Send(new MessengerErrorComposer(MessengerRequestError.FriendRequestsDisabled));
                return;
            }

            if (avatar.Messenger.Friends.Count >= avatar.Messenger.MaxFriendsAllowed)
            {
                avatar.Send(new MessengerErrorComposer(MessengerRequestError.FriendListFull));
                return;
            }
            ...
            if (targetAvatar != null)
                targetAvatar.Send(new NewBuddyRequestComposer(avatar.Details));
        }
